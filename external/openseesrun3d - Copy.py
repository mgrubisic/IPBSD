...
class OpenSeesRun3D:
    ...
    def rigid_diaphragm(self, spans_x, spans_y, nbays_x, nbays_y):
        ...
        for st in range(1, self.i_d.nst + 1):
            ...
            op.node(masternodetag, xa, zloc, ya)
            op.fix(masternodetag, 0, 1, 0, 1, 0, 1)
            ...
            for xbay in range(int(nbays_x + 1)):
                for ybay in range(int(nbays_y + 1)):
                    ...
                    op.rigidDiaphragm(2, masternodetag, nodetag)
        ...
        for xbay in range(int(nbays_x + 1)):
            ...
            for ybay in range(int(nbays_y + 1)):
                ...
                for st in range(1, self.i_d.nst + 1):
                    zloc += self.i_d.heights[st-1]
                    nodetag = int(f"{1+xbay}{1+ybay}{st}")
                    op.node(nodetag, xloc, zloc, yloc)
    ...
    def define_transformations(self):
        ...
        op.geomTransf("Linear", self.COL_TRANSF_TAG, 0, 0, 1)
        op.geomTransf("Linear", self.BEAM_X_TRANSF_TAG, 0, 0, 1)
        ...
        for st in range(1, self.i_d.nst + 1):
            for xbay in range(1, nbays_x + 2):
                for ybay in range(1, nbays_y + 2):
                    ...
                    op.mass(nodetag, mass, self.NEGLIGIBLE, mass, self.NEGLIGIBLE, self.NEGLIGIBLE, self.NEGLIGIBLE)
